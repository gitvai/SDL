        ...
        async function bulkAction(action, tbodyId = null) {
            ...
            if (action === 'print') {
                ids.forEach(id => {
                    if (tbodyId === 'prof-shipments-body' || (typeof currentMode !== 'undefined' && (currentMode === 'note' || currentMode === 'notes'))) {
                        window.open(`delivery_note.html?id=${id}`, '_blank');
                    } else {
                        window.open(`delivery_note.html?id=${id}&type=order`, '_blank');
                    }
                });
        ...
        function renderTable(data, type) {
            ...
            data.forEach((item, idx) => {
                ...
                if (type === 'client-summary') {
                    ...
                    const totalAmount = (item.orders || []).reduce((sum, o) => sum + (o.netAmount || o.totalAmount || 0), 0);
                    ...
                    row.innerHTML = `
                        ...
                        <td style="color:#6b7280;">${
                            ...
                            ? item.jobs.map(j => formatProductDisplay(j.productType, j.productName)).join(', ')
                            : formatProductDisplay(item.productType, item.productName)
                        }</td>
            ...
            try {
                ...
                ordersBody.innerHTML = (client.orders || []).map(o => `
                    <tr data-id="${o.id}">
                        ...
                        <td style="padding:10px; border-bottom:1px solid #eee; font-weight:700; color:#10b981;">${(o.netAmount || o.totalAmount || 0).toFixed(2)}</td>
                ...
                shipBody.innerHTML = (client.shipmentNotes || []).map((s, idx) => {
                    const amount = (s.orders || []).reduce((sum, o) => sum + (o.netAmount || o.totalAmount || 0), 0);
                    return `
                    <tr data-id="${s.id}">
        ...
        function sendPortalLink() {
            const portalUrl = `https://sdl-backend-l5r2.onrender.com/portal/${currentClientId}`;
                                ...
                                <div style="max-height:150px; overflow-y:auto;">
                                    ${data.orders.map(o => `
                                        <div style="display:flex; justify-content:space-between; font-size:12px; padding:4px 0; border-bottom:1px dotted #eee;">
                                            ...
                                            <span style="font-weight:bold;">OMR ${(o.netAmount || o.totalAmount || 0).toFixed(2)}</span>
                                        </div>
                                    `).join('')}
                                </div>
                                <div style="margin-top:10px; display:flex; justify-content:space-between; font-weight:bold; font-size:13px;">
                                    <span>Total Amount :</span>
                                    <span>OMR ${data.orders.reduce((sum, o) => sum + (o.netAmount || o.totalAmount || 0), 0).toFixed(2)}</span>
                                </div>
                ...
                items.forEach((item, idx) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td style="color:#6b7280;">${
                            ...
                            ? order.jobs.map(j => formatProductDisplay(j.productType, j.productName)).join(', ')
                            : formatProductDisplay(order.productType, order.productName)
                        }</td>
...
// --- GRID SETTINGS MODAL LOGIC ---
        ...
        function detectActiveType() {
            ...
            if (profileTabActive) {
                ...
                if ('shipments.html' === 'shipments.html') {
        ...
        if ('shipments.html' === 'shipments.html') {
            ALL_COLUMNS_DEF = {
                ...
                'note': [
                    { key: 'sr', name: '#', caption: '#' }, { key: 'noteNumber', name: 'Shipment #', caption: 'Shipment #' }, { key: 'orderNumber', name: 'Order #', caption: 'Order #' },
                    { key: 'noteDate', name: 'Shipment Date', caption: 'Shipment Date' }, { key: 'clientName', name: 'Client', caption: 'Client' },
        ...
        let gridUserFields = JSON.parse(localStorage.getItem('shipments.htmlFieldsSettings')) || {};
    ...
    <option value="Last 12 Months">Last 12 Months</option>
        ...
        window.saveGridSettings = function() {
            ...
            localStorage.setItem('shipments.html' + 'FieldsSettings', JSON.stringify(gridUserFields));
