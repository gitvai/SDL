            ...
            teeth.forEach(t => {
                ...
                if (circleColors.length > 0) {
                    ...
                    if (circleColors.length > 1) {
                        style += 'outline: 2px solid ' + circleColors[1] + '; outline-offset: 1px;';
                    }
                }
...
const renderJobsFunc = `function renderJobsList() {
...
function processFile(filename) {
    ...
    // Remove the old renderJobsList function block
    // We'll use a regex to replace it
    const renderPattern = /function renderJobsList\(\) \{[\s\S]*?(?=function cancelTeethSelection|\}\s*<\/script>|\}\s*function)/;
    
    // Check what the regex matches
    const match = html.match(renderPattern);
    if(match) {
        // Insert globalGridFunc and new renderJobsFunc
        html = html.replace(renderPattern, globalGridFunc + '\n' + renderJobsFunc + '\n        ');
        fs.writeFileSync(filename, html);
        console.log("Replaced renderJobsList in " + filename);
    } else {
        console.log("Could not find renderJobsList in " + filename);
    }
}
